...
Build out the Python CLI named `sn` from the Phase 1 archive plan.
...
- distribution package name `sn` so `uvx sn` works without `--from`
- tests run directly with `uv run python -m unittest discover -s tests`
- supported package `sn` under `src/`
- `sn sync` argument parsing and orchestration flow
- `sn status` text and JSON output
...
- no `__main__.py` module yet; the supported entry point is the `sn` console script
...
- `sn sync -v` now prints fetch progress to stderr for archive pages, transcript fetches, and stored files
...
- `sn sync` now supports `--year YYYY` to fetch a single yearly archive page
...
- `sn status` reports missing items, local counts, and general archive status
- non-present states are `remote_missing`, `fetch_error`, and `parse_error`
- `sn status` exits `2` when non-present states exist
